...
//      BOOL success2 = [_fmdb executeUpdate:@"CREATE TABLE IF NOT EXISTS bbx_tel("
//                        "id INTEGER PRIMARY KEY,"
//                        "class_id INTEGER,"
//                        "phone text,"
//                        "foreign key (contact_id) references  bbx_contact(id) on delete cascade on update cascade"
//         ")"];
...
- (BOOL)insertContact:(NSString *)uuid displayName:(NSString *)displayName contactName:(NSString *)contactName nickname:(NSString *)nickname nameRemark:(NSString *)nameRemark phoneNumbers:(NSString *)phoneNumbers emails:(NSString *)emails contactAddress:(NSString *)contactAddress ims:(NSString *)ims contactOrganization:(NSString *)contactOrganization birthday:(NSString *)birthday note:(NSString *)note photo:(NSString *)photo urls:(NSString *)urls{
     ...
     NSString *insertSql = [NSString stringWithFormat:@"INSERT INTO bbx_contact(uuid, displayName, name, nickname, nameRemark, phoneNumbers, emails, addresses, ims, organizations, birthday, note, photo, urls) VALUES ('%@', '%@','%@', '%@', '%@', '%@', '%@', '%@', '%@', '%@', '%@', '%@', '%@', '%@')", uuid, displayName,contactName,nickname,nameRemark, phoneNumbers, emails, contactAddress, ims, contactOrganization, birthday, note, photo, urls];
