...
-(NSArray *)sortedLocalContact
{
    ...
    for (int i=0; i < localContact.count; i++) {
        ...
        NSDictionary *dic = @{keyPointer: value,keyName:pinyinName};
        [contactSortedArr addObject:dic];
    }
    
    
    NSSortDescriptor *sortDes = [[NSSortDescriptor alloc] initWithKey:keyName ascending:YES];
    ...
    for (int i=0; i < contactSortedArr.count; i++) {
        ...
        NSInteger position = [dic[keyName] characterAtIndex:0] - 'A';
        if (position < 0) {
...
//        contact.dic = dic;
        ...
        contactName.pinName = dic[keyName];
